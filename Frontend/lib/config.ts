...
const productionConfigurationIsUnsafe =
  ...
  (!normalizedConfiguredBackendUrl || configuredBackendIsLoopback);
...
const BACKEND_URL = (
  productionConfigurationIsUnsafe
    ? "" // No hardcoded fallback: fail loudly instead of calling a dead URL
...
if (!rawBackendUrl) {
  console.warn(
    `[FrontendConfig] NEXT_PUBLIC_BACKEND_URL is not set. Falling back to ${BACKEND_URL || "(none)"}.`
  );
  console.warn(
...
if (productionConfigurationIsUnsafe) {
  console.error(
    "[FrontendConfig] NODE_ENV is production but NEXT_PUBLIC_BACKEND_URL is missing or points at localhost."
  );
  console.error(
    "[FrontendConfig] Set NEXT_PUBLIC_BACKEND_URL in the Render frontend service to the deployed backend origin, then redeploy."
  );
}
